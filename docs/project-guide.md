...
本文档用于项目复盘、技术交流和对外说明，目标是把本项目讲成一个完整、清晰、有工程深度的 AI 应用实践项目。

## 1. 一分钟项目介绍

可以这样概括：

> 这个项目是一个 Spring Boot + Spring AI 实践项目，业务场景模拟金融信贷助手，覆盖普通对话、Function Calling、RAG 知识库问答、混合检索、重排序、Agent Loop、Prompt 版本管理和 AI 调用成本统计。
>
> 项目重点不是简单调一个模型接口，而是练习大模型在真实后端系统中的落地方式。用户问题会先经过意图路由，业务查询类问题走工具调用，知识问答类问题走 RAG，复杂问题可以走 Hybrid 或 Agent 模式。Agent 模块显式实现了 Plan、Tool、Reflect、Replan、SelfCheck、Respond 的执行闭环。
>
> 通过这个项目，可以系统展示 Spring AI 的 ChatClient、工具调用、RAG 管道、Agent 编排、PromptOps、成本观测和工程分层设计。
...
>
> 所以项目用金融信贷助手作为场景，把用户问题分成几类：如果是借款、还款、风险评估，就通过 Function Calling 调用后端数据库；如果是产品规则、风控政策，就通过 RAG 检索知识库；如果是复杂问题，就通过 Agent Loop 先规划、再调用工具、再反思和自检。

补充说明：

> 这个项目更像一个 AI 应用开发知识点的综合练习，而不是单一业务系统。它帮助理解 Chat、Tool、RAG、Agent、PromptOps 和 Observability 之间的关系。
...
> 当前项目先用规则和关键词，是因为它更稳定、成本低、可解释，也适合学习阶段观察路由效果。生产环境可以进一步演进成规则 + 小模型分类 + LLM fallback 的组合方式。

### Q3：Function Calling 是怎么实现的？

> Chat 入口使用 Spring AI 的 `@Tool` 机制，把借款查询、还款查询、风险评估注册为模型可调用工具。模型根据工具描述决定是否调用，并生成结构化参数。
>
> 重构后真实业务逻辑下沉到了 `service.business`，比如 `LoanQueryService`、`RepaymentQueryService`、`RiskAssessmentService`。Spring AI 的 Function 类只是适配器，Agent Tool 也是适配器。这样同一份业务能力既能被 Spring AI 自动工具调用，也能被自研 Agent ToolRouter 调用。

主要风险：

> 模型可能选错工具、参数不完整、返回数据太大、敏感操作不受控。所以项目里工具只做查询类操作，Agent 侧还有工具白名单，后续可以继续加参数校验、权限校验和敏感操作确认。

### Q4：RAG 流程是怎样的？

> 项目的 RAG 分为离线建库和在线检索两部分。
>
> 离线阶段，系统从 `src/main/resources/knowledge` 加载业务规则、产品手册、风控指南等文档，补充元数据，然后用 `TokenTextSplitter` 切分，再通过 embedding 模型生成向量，写入向量库。
>
...
> 自研一遍可以更清楚地理解规划、工具执行、观察、反思、自检和兜底，而不是把它当成黑盒。生产环境可以再考虑引入成熟 Agent 框架。
...
### Q14：Chat、RAG API、Agent 三个入口有什么区别？
...
>
> 所以项目做了一次面向学习和项目表达的重构：把向量检索统一到 `VectorRetrievalService`，把 Agent Loop 拆成 Planner、ToolExecutor、Reflector、Responder、Policy、State，把业务工具能力下沉到 `service.business`。这样架构更清晰，也更容易解释每一层的职责。
...
### 为什么使用本地向量存储，而不是 Milvus、Qdrant、ES？

> 这个项目主要用于学习和本地实验，本地向量存储部署简单，不依赖外部向量数据库，适合快速跑通核心流程。
>
> 如果上生产，可以考虑 Milvus、Qdrant、Elasticsearch 或 PGVector，重点关注数据规模、过滤能力、召回性能、运维复杂度和成本。
...
问题：“用户问系统：请评估 CUST1001 的风险，会发生什么？”
...
> 这是一个用 Spring Boot + Spring AI 实现的大模型应用开发综合练习项目，通过金融信贷助手场景，把 Chat、Function Calling、RAG、Agent Loop、PromptOps 和成本观测串成了一套可运行、可解释、可展示的后端系统。
